  ...
  return (
    <ul className="inline-flex h-10 overflow-hidden rounded-md text-base border border-indigo-400/40">
      {pages.map(page => {
        ...
        if([ELLIPSIS_LEFT, ELLIPSIS_RIGHT].includes(page)){
          return (
        ...
        return(
        <li key={page} className={"border-x border-indigo-400/40 first:rounded-l last:rounded-r first:border-0 last:border-0"}>
          <Link href={url} className={cn("flex h-10 items-center justify-center bg-slate-700 hover:bg-indigo-400/40 px-4 hover:text-slate-300", { "bg-indigo-400/50 text-slate-300": isCurrentPage })}>{page}</Link>
        </li>
      )})}
    </ul>
  )
